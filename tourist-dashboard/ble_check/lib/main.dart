...
class _BluetoothHomePageState extends State<BluetoothHomePage> {
  ...
  Future<void> _selectDeviceAndConnect() async {
    try {
      ...
      final BluetoothDevice? selectedDevice =
        ...
        context: context,
        builder: (context) => const _DevicePickerDialog(),
      );
  ...
  void _handleLine(String line) {
    // avoid heavy setState calls; batch changes
    ...
    setState(() {
      ...
      if (parsed != null) {
        _lastJson = parsed;
        _status = "Received JSON: ${parsed['type'] ?? 'unknown'}";
      } else {
  ...
  @override
  Widget build(BuildContext context) {
    final lastType = _lastJson?['type']?.toString() ?? '-';
